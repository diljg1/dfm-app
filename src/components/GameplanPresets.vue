<template>
    <div class="uk-flex uk-flex-middle">
        <div class="uk-flex-1 ws-dropdown-gameplans">
            <button
                class="
                    uk-button uk-button-default uk-width-1-1@m uk-text-truncate
                "
                type="button"
            >
            ...
            <div ref="dropdown" class="dfm-dropdown-scroll">
                <ul class="uk-nav uk-dropdown-nav">
                    <li
                        v-for="{ preset, active } in hashedPresets"
                        :key="preset"
                    ...
                    <li
                        v-for="(gameplan, index) in hashedPersonalGameplans"
                        :key="index"
                        :class="{ 'uk-active': gameplan.active }"
                    >
                        <a
                            href="#"
                            @click.prevent="setPersonalPresetParams(index)"
                        >
                            <span class="uk-flex uk-flex-middle">
                                <span
                                    class="uk-flex-1"
                                    :class="{
                                        'uk-text-muted': !gameplan.hasParams,
                                    }"
                                >
                                    {{ gameplan.name }}
                                </span>
                                <span>
                                    <a
                                        :title="
                                            $trans('Sla huidige parameters op')
                                        "
                                        class="uk-margin-small-left"
                                        uk-tooltip="delay: 200"
                                        @click.stop.prevent="
                                            savePresetParams(index, gameplan)
                                        "
                                    >
                                        <span
                                    ...
                                    <a
                                        :title="$trans('Bewerk naam')"
                                        class="uk-margin-small-left"
                                        uk-tooltip="delay: 200"
                                        @click.stop.prevent="
                                            editPresetName(index, gameplan)
                                        "
                                    >
                                        <span
                                    ...
                                    <a
                                        :title="$trans('Verwijder gameplan')"
                                        class="uk-margin-small-left"
                                        uk-tooltip="delay: 200"
                                        @click.stop.prevent="
                                            removePreset(index)
        ...
        <div style="width: 90px" class="uk-text-right">
            <button
                :title="$trans('Voeg eigen gameplan toe')"
...
export default {
    ...
    computed: {
        ...
        activeGameplan() {
            return [
                ...this.hashedPresets,
                ...this.hashedPersonalGameplans,
            ].find(({ active }) => active);
        },
        activeGameplanName() {
            if (this.activeGameplan) {
                return this.activeGameplan.preset
                    ? `gp.${this.activeGameplan.preset}`
                    : this.activeGameplan.name;
            }
        ...
        personalGameplans: {
            ...
            set(value) {
                this.$store.commit(SET_USERFIELD, {
                    field_name: 'gameplans',
                    value,
                });
            },
        },
    ...
    methods: {
        ...
        applyGameplanToParams(gameplan) {
            const [watchList = 'Safe', ownWatchlistId = 0] =
                gameplan.watchList || [];
            this.$store.commit(SET_PARAMS, {
                ...this.params,
                ...gameplan.params,
            });
            this.$store.commit(types.SET_PARAM, {
                name: 'Watchlists',
                value: watchList,
            });
        ...
        getGameplanHash(gameplan) {
            const [watchList = 'Safe', ownWatchlistId = 0] =
                gameplan.watchList || [];
            return md5(
                JSON.stringify(gameplan.params) +
                    `:${watchList}:${ownWatchlistId}`
            );
        },
        async addPreset() {
            const name = await this.promptName(
                `Gameplan ${this.personalGameplans.length + 1}`
            );
            if (name) {
        ...
        async removePreset(index) {
            this.personalGameplans = arrayWithRemovedItem(
                this.personalGameplans,
                index
            );
        ...
        async promptName(defaultName) {
            const prompt = UIkit.modal.prompt(
                this.$trans('Naam gameplan:'),
                defaultName
            );
        ...
        async editPresetName(index, gameplan) {
            try {
                ...
                if (name) {
                    this.personalGameplans = arrayWithReplacedItem(
                        this.personalGameplans,
                        index,
                        {
                            ...gameplan,
                ...
                this.$notify(
                    this.$trans('Fout bij aanpassen naam'),
                    'danger',
                    'warning'
                );
            }
        ...
        async saveGameplans(successMessage = '') {
            try {
                ...
                const success = await this.updateUserField({
                    field_name: 'gameplans',
                    value,
                });
                if (success) {
                    this.$notify(
                        successMessage || this.$trans('Gameplan opgeslagen'),
                        'success',
                        'check'
                    );
                } else {
                    this.$notify(
                        this.$trans('Fout bij opslaan gameplans'),
                        'danger',
                        'warning'
                    );
                }
